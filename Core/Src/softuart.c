...
extern uint8_t soft_serial_command[64];
#define CMD_BUFFER_SIZE 64 // size of the input buffer
char su_cmd_buffer[CMD_BUFFER_SIZE];
int8_t su_cmd_buffer_index = 0;
...
void SoftUartReadLine(uint8_t uart_num)
{
  static uint8_t Buffer[CMD_BUFFER_SIZE],Len;
  Len = SoftUartRxAlavailable(uart_num);
  if(Len==0){
	return;
  }
	
  if(SoftUartReadRxBuffer(0,Buffer,Len)!=SoftUart_OK){
	logSerial("Error: unable to read command from soft uart serial");
	return;
  }
  ...
  for(int i=0; i<Len; i++){
     if(Buffer[i] == '\n'){
         ...
         memcpy(soft_serial_command, su_cmd_buffer, su_cmd_buffer_index);  // copy the data to the buffer
         su_cmd_buffer_index=0;
     }else{
        su_cmd_buffer[su_cmd_buffer_index] =  Buffer[i];
        su_cmd_buffer_index++;
     }
     if(su_cmd_buffer_index>CMD_BUFFER_SIZE){
       su_cmd_buffer_index=0;
       logSerial("Error: Command size exceeded!");
     }
  }
  
  memset(Buffer, '\0', Len);   // clear Buf 
}
